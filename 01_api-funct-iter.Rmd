...
```{r}
y1980 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1980")
y1999 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/1999")
y2018 <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/total/2018")
```
...
```{r}
visitation <- httr::GET(url = "https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=ROMO&startMonth=JANUARY&startYear=2010&endMonth=DECEMBER&endYear=2021")
```
...
```{r, eval = F}
parkwide_visitation <- function(yr){
  url <- paste0("https://irmaservices.nps.gov/v3/rest/stats/total/", yr)
  returnYear <- httr::GET(url = url)
  return(returnYear)
  }
```
...
```{r}
unit_visitation <- function(parkCode, startYear, endYear, startMonth, endMonth){
...
```{r}
romo <- unit_visitation("ROMO", "1990","2021","01","12")
ever <- unit_visitation("ever", "1990","2021","01","12")
```
...
romo <- unit_visitation("romo")
ever <- unit_visitation("ever")
thro <- unit_visitation("thro")
```
...
for(i in 1:length(years)){
  output_floop[[i]] <- parkwide_visitation(years[i])
...
parks_output <- vector("list", length = length(parks))
for (i in 1:length(parks)) {
  parks_output[[i]] <- unit_visitation(parks[[i]])
}
multi_park <- dplyr::bind_rows(parks_output)
summary(multi_park)
...
```{r}
park_unit_visit <- parks %>% map(~ unit_visitation(parkCode = .))
park_unit_visit <- bind_rows(park_unit_visit); summary(park_unit_visit)
```
