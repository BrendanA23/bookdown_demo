...
```{r}
co_water <- function(site, start = 2021, end = 2022){
#site = "CLAFTCCO"; start = 2021; end = 2022;
raw <- GET(url = paste0(
  "https://dwr.state.co.us/Rest/GET/api/v2/surfacewater/surfacewatertsday/?format=",
  "json&dateFormat=dateOnly&fields=abbrev%2CmeasDate%2Cvalue%2CmeasUnit&encoding=deflate&abbrev="
  ,site,
  "&min-measDate=10%2F01%2F",start - 1, 
  "2020&max-measDate=09%2F30%2F", end))
txt_fmt <- httr::content(raw, as = "text", encoding = "UTF-8")
js <- jsonlite::fromJSON(txt_fmt)
...
```{r}
cwater <- c("CLAFTCCO", "CLARIVCO")
cwater <- cwater %>% map(~ co_water(site = .))
#couldn't get the ApI pulling the dates right so Im sub-setting for the correct range
s1 <- subset(cwater[[1]][["ResultList"]], measDate > "2020-10-00")
s2 <- subset(cwater[[2]][["ResultList"]], measDate > "2020-10-00")
cdwr <- bind_rows(s1,s2)
```
...
```{r}
#cdwr: abbrev   measDate value measUnit ::: agency_cd  site_no       Date q_cfs X_00060_00003_cd    Site
usgs <- usgs %>% select(Site, Date, q_cfs) %>% rename(site = Site, date = Date, q_cfs = q_cfs)
cdwr <- cdwr %>% select(abbrev, measDate, value) %>% rename(site = abbrev, date = measDate, q_cfs = value)
usgs$date <- as.character(usgs$date)
water <- bind_rows(usgs, cdwr)
head(water)
...
plotly::ggplotly(
  ggplot(water)+
    ...
    theme_classic() + ylab("discharge (CFS)") + ggtitle("Cache la Poudre River flow")
)
```
...
```{r}
dis_diff <- water %>% pivot_wider(., names_from = site, values_from = q_cfs)
d <- dis_diff

for (i in 1:length(d$date)) {
  d$Lincoln[i]  <-   d$CLAFTCCO[i] - d$Lincoln[i]
  d$Boxelder[i] <-   d$CLAFTCCO[i] - d$Boxelder[i]
  d$CLARIVCO[i] <-   d$CLAFTCCO[i] - d$CLARIVCO[i]
  d$CLAFTCCO[i] <-   d$CLAFTCCO[i] - d$CLAFTCCO[i]
}
head(d)
d <- d %>% pivot_longer(cols = -date, names_to = "site", values_to = "q_cfs") 
plotly::ggplotly(
  ggplot(d)+
    geom_point(aes(x = date, y = q_cfs, color = site))+
    scale_x_date(date_breaks = "3 month", date_labels =  "%b %Y")+
    theme_classic() + ylab("discharge difference (CFS)") + ggtitle("Daily Discharge Difference: Canyon Mouth vs Sites")
)
```
...
e <- subset(d, q_cfs < 0)
e <- c <- unique(e$date)
length(e) # == 85 days where the canyon mouth had a lower flow than the downstream sites
```
There was 85 days where the canyon mouth had a lower flow than the downstream sites, I did not expect this many days since the canyon seems to be the source of water for the rest of the sites, but I guess they release water from Horsetooth occasionally and that might mess with things. 
