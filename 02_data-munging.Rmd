...
```{r}
unit_visitation <- function(parkCode, startYear = "1980", endYear = "2021", startMonth ="01", endMonth="12"){
  link <- paste0("https://irmaservices.nps.gov/v3/rest/stats/visitation?unitCodes=",parkCode, "&startMonth=",startMonth,"&startYear=",startYear,"&endMonth=",endMonth,"&endYear=",endYear)
  
  uVis <- httr::GET(url = link)
# convert content to text
  uVisTxt <- httr::content(uVis, as = "text", encoding = "UTF-8") 
# parse text from JSON to data frame
  returnData <- jsonlite::fromJSON(uVisTxt)
return(returnData)
}
units <-c("ROMO", "ACAD", "LAKE", "YELL", "GRCA", "ZION", "OLYM", "GRSM")
units <- units %>% map(~ unit_visitation(parkCode = .))
...
labls <- c('FALSE' = "Individual Parks", 'TRUE' = "ParkWide")
plotly::ggplotly(
  ggplot(data=yearly) +
    geom_point(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    geom_path(aes(x = Year, y = RecVisitation, color = UnitCode)) +
    scale_y_continuous(labels = scales::label_scientific()) +
    facet_wrap(~UnitCode == "Parkwide", scales = "free_y", labeller = as_labeller(labls)) +
    theme_bw(base_size=10)
)
...
```{r}
unitPct <- function(x, r){ 100*(x/wide_data$Parkwide[[r]])}
pct_wide <- wide_data
for (i in 1:length(pct_wide)) {
  pct_wide$ACAD[i] <- unitPct(pct_wide$ACAD[i], i)
  pct_wide$ROMO[i] <- unitPct(pct_wide$ROMO[i], i)
  pct_wide$LAKE[i] <- unitPct(pct_wide$LAKE[i], i)
  pct_wide$YELL[i] <- unitPct(pct_wide$YELL[i], i)
  pct_wide$GRCA[i] <- unitPct(pct_wide$GRCA[i], i)
  pct_wide$ZION[i] <- unitPct(pct_wide$ZION[i], i)
  pct_wide$OLYM[i] <- unitPct(pct_wide$OLYM[i], i)
  pct_wide$GRSM[i] <- unitPct(pct_wide$GRSM[i], i)
  
}
annPct <- pct_wide %>%
  pivot_longer(cols = -Year,names_to = "Park", values_to = "Percentage of total visitation") %>%
  filter(!Park == "Parkwide")
annPct$`Percentage of total visitation` <- as.numeric(annPct$`Percentage of total visitation`); annPct

plotly::ggplotly(
  ggplot(annPct)+
    geom_point(aes(x= Year, y = `Percentage of total visitation`, color = Park))+
    geom_path(aes(x= Year, y = `Percentage of total visitation`, color = Park))+
    theme_classic()
)
```
...
typeof(units)
typeof(parkwide)
units <- bind_rows(units, .id = "column_label")
...
```{r}
joined_data <- joined_data %>% mutate( percentage = 100*(RecreationVisitors.x / NonRecreationVisitors.y))
plotly::ggplotly(
  ggplot(joined_data)+
  geom_point(aes(x = Year, y = percentage, color = UnitName.x))+
  theme_classic()
  )
...
```{r}
data_outliers <- subset(yearly, Year >= 1990) %>% 
  filter(UnitCode != "Parkwide") %>%
  select(Year, UnitCode, RecVisitation) %>%
  pivot_wider(., names_from = UnitCode, values_from = RecVisitation)

park_avg <- data_outliers %>% 
  mutate_at(.vars = c("ACAD", "ACAD","GRCA","GRSM","LAKE","OLYM","ROMO","YELL","ZION"),
            .funs = ~ min(.)) %>% 
  pivot_longer(.,cols = -Year,names_to = "Park", values_to = "average_visitation") %>%
  select(Park,average_visitation )

head(park_avg)
##ACAD    GRCA    GRSM    LAKE    OLYM    ROMO    YELL    ZION
#2051484 2897098 8151769 6285439 2499177 2647323 2758526 2102400

ggplot(park_avg, aes(Park,average_visitation))+
  geom_col()+ theme_classic() + ggtitle("Average Visitation")
```

ACAD had the least average visitation: 2051484 
GRSM had the most average visitation:  8151769
